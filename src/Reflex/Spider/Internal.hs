...
data SwitchSubscribed x a
   = SwitchSubscribed { switchSubscribedCommon :: !(CommonSubscribed (SwitchSubscribed x) x a)
...
data CoincidenceSubscribed x a
   = CoincidenceSubscribed { coincidenceSubscribedCommon :: !(CommonSubscribed (CoincidenceSubscribed x) x a)
...
data CommonSubscribed s x a
   = CommonSubscribed { commonSubscribedCachedSubscribed :: !(IORef (Maybe (s a)))
                      , commonSubscribedWeakSelf :: !(IORef (Weak (s a)))
...
getSwitchSubscribed s sub = do
  getCommonSubscribed (switchSubscribed s) switchSubscribedCommon (finalize <=< readIORef . switchSubscribedOwnWeakInvalidator) (readIORef . switchSubscribedCurrentParent) sub
    $ \subscribedUnsafe -> do
      ...
      pure (parentOcc, height, \common ->
                                   SwitchSubscribed
                                   { switchSubscribedCommon = common
...
{-# INLINE getCommonSubscribed #-}
-- | This is code originally factored out from getCoincidenceSubscribed and getSwitchSubscribed.
-- TODO: why is there subscribedRef__ and subscribedRef?
getCommonSubscribed :: forall (x :: Type) s a. HasSpiderTimeline x =>
  IORef (Maybe (s a)) ->
  (s a -> CommonSubscribed s x a) ->
  (s a -> IO ()) ->
  (s a -> IO (EventSubscription x)) ->
  Subscriber x a ->
  (s a -> EventM x (Maybe a, Height, CommonSubscribed s x a -> s a)) ->
  EventM x (WeakBagTicket, s a, Maybe a)
getCommonSubscribed subscribedRef__ getSCommonSubscribed extraCleanup whatToUnsubscribeOnCleanup sub foo = do
  mSubscribed <- liftIO $ readIORef $ subscribedRef__
  let cleanup subscribed = do
        unsubscribe =<< whatToUnsubscribeOnCleanup subscribed
        extraCleanup subscribed
        writeIORef ((commonSubscribedCachedSubscribed . getSCommonSubscribed) subscribed) Nothing
  case mSubscribed of
...
getCoincidenceSubscribed c sub = do
  getCommonSubscribed (coincidenceSubscribed c) coincidenceSubscribedCommon (const (pure ())) (pure . coincidenceSubscribedOuterParent) sub $ \subscribedUnsafe -> do
      ...
      pure (occ, height, \common ->
                           CoincidenceSubscribed
                            { coincidenceSubscribedCommon = common
...
mergeCheap nt =
  ...
  where
      ...
      getInitialSubscriber :: MergeInitFunc k v q x (MergeSubscribedParent x)
      getInitialSubscriber = pure . (, MergeSubscribedParent) . pure

-- Type-checker isn't happy when lambda is avoided:
{-# ANN mergeCheapInlined "HLint: ignore Avoid lambda" #-}
{-# INLINE mergeCheapInlined #-}
mergeCheapInlined :: forall k x q v. (HasSpiderTimeline x, GCompare k)
  => (forall a. q a -> Event x (v a))
  -> DynamicS x (PatchDMap k q)
  -> Event x (DMap k v)
mergeCheapInlined nt d = Event $ \sub -> do
  ...
  defer $ SomeMergeInit $ do
    ...
    -- TODO: are there other cases where something like "forM_ change"
    -- is the same as the "propagate" from the subscriber? Originally
    -- this was not written like this. The tracePropagate only
    -- triggered on the subscribeAndRead version but this is a no-op
    -- so morally the same. Maybe tracePropagate could wrap subscribers.
    forM_ change (subscriberPropagate changeSubscriber)
...
mergeCheapWithMove nt =
  ...
  where
      ...
      getInitialSubscriber :: MergeInitFunc k v q x (MergeSubscribedParentWithMove x k)
...
type MergeInitFunc k v q x s
   = forall a. k a -> EventM x  (EventM x (k a), EventSubscription x -> s a) -- (Maybe (DSum k v), Height, DSum k s)
...
mergeGCheap' nt getInitialSubscriber updateFunc getSub d = Event $ \sub -> do
  ...
  (dm, heights, initialParentState) <- do
        subscribers <- forM (DMap.toList initialParents) $ \(k :=> e) -> do
          ...
          (s, mkSub) <- getInitialSubscriber k
  ...
  defer $ SomeMergeInit $ do
    ...
    liftIO $ writeIORef changeSubdRef (changeSubscriber, changeSubscription)
  let unsubscribeAll =
          mapM_ (unsubscribe . (\(_ :=> s') -> getSub s')) . DMap.toList =<< readIORef parentsRef
